...
* Escribiendo Código Específico para Android 

Flutter proporciona un framework gneral para acceder a características específicas de una plataforma. Esto habilita a el desarrollador para extender la funcionalidad del framework /Flutter/ usando código específico de la plataforma. Funcionalidades específicas de la plataforma como la cámara, batería, navegador, etc, pueden ser accesibles fácilmente a través del framework. 

La idea general de acceso al código específico de plataforma es a través de un simple protocolo de mensajería. El código de Flutter, Cliente y el código de la plataforma y los enlaces al hospedador en un canal común de Mensajería. El cliente envía mensajes al hospedador a través  del Canal de Mensajería. El hospedador escucha en el Canal de Mensajería, recive el mensaje y hace la funcionalidad necesaria y finalmente, devuelve el resultado al cliente a través del Canal de Mensajería. 

El código de arquitectura específico de plataforma es mostrada en el diagrama a continuación - 

[[./images/canal-mensajeria.png]]


El protocolo de Mensajería usa un codec estándar de Mensajería (clase /StandardMessageCodec/) que soporta la serialización de binarios de valores tipo JSON tales como números, cadenas de texto, booleanos, etc. La serialización y la deserealización funciona de forma transparente entre el cliente y el hospedador. 

Vamos a escriir una aplicación simple para abrir un navegador usando el SDK de Android y entendiendo cómo - 

+ Creamos una nueva aplicación /Flutter/ llamada, /flutter_browser_app/
+ Reemplazamos el código de main.dat por el siguiente - 
...
+ Aquí hemos creado un nuevo botón para abrir el navegador y establecemos que el método /onPressed/ como null.
+ Ahora, importamos los siguientes paquetes - 
...
+ /services.dart/ incluye la funcionalidad de invocar código específico de plataforma.
+ Crearemos un nuevo canal de Mensajería en el widget /MyHomePage/.

#+BEGIN_SRC dart 
  static const platform = const
  Method Channel('flutterapp.tutorialspoint.com/browser');
#+END_SRC

Escribimos un método /_openBrowser/ para invocar el método específico de plataforma a través del canal de Mensajería. 
...
Aquí hemos usado /platform.invokeMethod/ para invocar /openBrowser/ (explicado en los siguientes pasos). /openBrowser/ tien un argumento, /url/ para abrir una url especifica. 

+ Cambiamos el valor de la propiedad /onPressed/ en /RaisedButton/ de null a /_openBrowser/. 

#+BEGIN_SRC dart 
onPressed: _openBrowser,
#+END_SRC

+ Abrimos /MainActivity.java/ (dentro de la carpeta Android) e importamos la bibliotecas requeridas. 
...
+ Escribimos un método, /openBrowser/ para abrir el navegador 
...
#+BEGIN_SRC java 
  private void openBrowser(MethodCall call, Result result, String url){
	  ...
	  result.error("ACTIVITY_NOT_AVAILABLE",
		       "Browser cannot bbe opened without foreground
		       activity", null);
	  return;
      }
...
+ Ahora, establecemos el nombre del canal en la clase /MainActivity/ - 

#+BEGIN_SRC java 
  private static final String CHANNEL = "flutterapp.tutorials.point.com/browser";
#+END_SRC

+ Escribimos un código específico de android que establece el mensaje manejado en el método /onCreate/ - 
...
Aquí, hemos creado un canal de Mensajería usando la clase /MethodChannel/ y usando la clase /MethodCallHandler/ para manejar el mensaje. /onMethodCall/ es el verdadero método responsable para llamar al código específico de plataforma comprobando el mensaje. El método /onMethodCall/ extrae la url del mesnaje y entonces invoca /openBrowser/ solo cuando el método llamado es /openBrowser/. De otra forma, devolverá el método /notImplemented/. 
...
#+BEGIN_SRC java 
  ...
  public class MainActivity extends FlutterActivity {
	  ...
	  Activity activity = this; if (activity == null) {
	      result.error(
			   "ACTIVITY_NOT_AVAILABLE", "Browser cannot be openned without foreground activity", null
			   );
	      return;
	  }
	  Intent intent = new Intent(Intent.ACTION_VIEW);
...
Ejecutamos la aplicación y pinchamos en el botón /Open Browser/ y veremos como se lanza el navegador. Vemos una imagen de cómo quedaría. 

[[./images/demo-navegador.png]]

[[./images/demo-navegador2.png]]
